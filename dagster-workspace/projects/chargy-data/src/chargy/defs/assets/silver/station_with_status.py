...
ASSET_KEY_PARQUET = dg.AssetKey(["bronze", "chargy", "station"])
...
@dg.asset(
    ...
    group_name="silver",
    description="All station readings, with description of the station and its status",
    io_manager_key="polarsiceberg_iomanager",
    metadata={
        "dagster/table_name": "chargy.station_with_status",
        "partition_expr": "date",
    },
    backfill_policy=dg.BackfillPolicy.multi_run(max_partitions_per_run=1),
    # automation_condition=dg.AutomationCondition.eager().without(
    #     dg.AutomationCondition.in_latest_time_window()
    # ),    # Creates backfill runs, and dagster-iceberg is not compatible with it (no partition_key in OutputContext in the IO Manager)
)
def station_with_status(
    ...
    if historical_postgres is not None:
        historical_postgres = historical_postgres.with_columns(
            ...
            pl.col("cpnum").cast(pl.Int64).alias("cpnum"),
        )
...
silver_station_selection = dg.AssetSelection.assets(
    dg.AssetKey(["silver", "chargy", "station_with_status"])
)

materialize_silver_station_job = dg.define_asset_job(
    name="materialize_monthly_station_silver", selection=silver_station_selection
)


@dg.sensor(
    name="trigger_silver_station_sensor",
    job=materialize_silver_station_job,
    default_status=dg.DefaultSensorStatus.RUNNING,
)
def trigger_silver_station_sensor(context: dg.SensorEvaluationContext):
    ...
    for asset_key in UPSTREAM_ASSET_KEYS:
        ...
        context.log.info(
            f"Added {len(asset_key_events)} events for asset '{asset_key.to_user_string()}'"
        )

    if len(asset_records) == 0:
        context.log.info("No new events found")
        return
    ...
    for tup in asset_records:
        if (
            ...
            dat = _to_date(tup[0].partition_key)
    ...
    for mth, dat_set in month_dict.items():
        if len(dat_set) == _nb_days_in_month(mth) or mth == last_partition_key:
...
def _nb_days_in_month(mth_key: str) -> int:
...
class SilverTimeWindowPartitionMapping(dg.PartitionMapping):
    def get_downstream_partitions_for_partitions(
        self,
        upstream_partitions_subset: PartitionsSubset,
        upstream_partitions_def: dg.PartitionsDefinition,
        downstream_partitions_def: dg.PartitionsDefinition,
        current_time: Optional[dt.datetime] = None,
        dynamic_partitions_store: Optional["DynamicPartitionsStore"] = None,
    ) -> PartitionsSubset:
        upstream_time_window_partition: dg.TimeWindowPartitionsDefinition = (
            upstream_partitions_def  # pyright: ignore[reportAssignmentType]
        )
        downstream_time_Window_partition: dg.TimeWindowPartitionsDefinition = (
            downstream_partitions_def  # pyright: ignore[reportAssignmentType]
        )

        end_dt = (
            downstream_time_Window_partition.end
            if downstream_time_Window_partition.end
            else dt.datetime.now(dt.timezone.utc)
        )

        intersection = (
            upstream_time_window_partition.get_partition_subset_in_time_window(
                dg.TimeWindow(downstream_time_Window_partition.start, end_dt)
            )
        )
